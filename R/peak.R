...
newPeak <- function(name, chr, start, end, cellType=NULL, modification, maxPeak, score=NULL, probes=c(), ...) {
  peak <- list(name=name, chr=chr, start=start, end=end, cellType=cellType, modification=modification, typeUpstream=list(), typeDownstream=list(), maxPeak=maxPeak, score=score, probes=probes)
...
print.peak <- function(x, ...) {
  ...
  with(x, if (!is.null(modification)) cat("Modification :",modification,"\n"))
...
generatePeakList = function(peaks,gff, g2t=NULL, allChr=c(1:19, "X", "Y"), tssCover=c(-25, 25), tssUpstream=5000, tssDownstream=20000, cellType="HL1", verbose=TRUE) {
  ...
  index = 1
  for (modification in names(peaks)) {
    if (verbose) cat("\n| Processing modification", modification, "\n+-- chr")
    curMod = peaks[[modification]]
    for (chr in allChr) {
      if (verbose) cat("", chr)
      curChr = curMod[[chr]]
      ...
      generatePeak = function(pName) {
        ...
        return(newPeak(peakID=pName, chr, start=as.integer(n[1]), end=as.integer(n[length(n)]), cellType=cellType, modification=modification, maxPeak=max(p), score=this.peak.score))
      }# generatePeak
