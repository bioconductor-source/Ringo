...
\section{Mapping probes to genomic coordinates}

By \emph{probes}, we mean the oligo-nucleotides or PCR products
...
Each probe has a unique identifier and (hopefully) a unique sequence,
but can, and probably does, appear in multiple copies as \emph{features}
on the array surface.

A mapping of probes to genomic coordinates is usually provided
by the array manufacturer, such as in NimbleGen's *.POS files. If the
probe sequences are provided as well, you may consider to perform a custom
mapping of probe sequences to the genome of interest, using alignment
tools such as \emph{Exonerate} \citep{Slater2005}. 
Such a re-mapping of probes to the genome
can sometimes be necessary, for example when the array has designed on
an outdated assembly of the genome.
Re-mapping also provides the advantage 
that you can allow non-perfect matches of probes to the genome,
if desired. There are numerous indications that perfect sequence
complementarity might not be required for hybridization
\citep[see, e.g.,][for a recent review on the subject]{Pozhitkov2007}.
For example, if a probe sequence of 50 nucleotides length matches a
genomic stretch on one chromosome with 49 out of the 50 nucleotides, one
may wonder whether immuno-precipitated fragments containing this 
genomic region will not hybridize to the probe.

Once probes have been mapped to the genome, this mapping needs to
...
Whether a probeAnno environment will work with other \Rpackage{Ringo}
functions can be checked using the function \Rfunction{validProbeAnno}
The function \Rfunction{posToProbeAnnoEnvironment} allows generation
of such a probeAnno environment, 
...
Furthermore, the package's \texttt{scripts} directory contains 
a script \texttt{makeProbeAnno.R} that demonstrates how to 
generate such a mapping object either from a NimbleGen \texttt{POS} file
and Perl scripts that allow the conversion of multiple output files from
common alignment tools such as Exonerate into one file that corresponds
to a POS file.
The function \Rfunction{validProbeAnno} can be used to perform a
quick check whether a generated probeAnno environment will probably
...
@ 
<<showPeaks>>=
peaksXD[order(peaksXD$maxPeak, decreasing=TRUE),]
@ 
 
One characteristic of enriched regions that can be used for 
sorting them is the element \texttt{maxPeak}, that is the highest smoothed
...
threshold. Another characteristic of enriched regions that can be used for 
sorting them is the element \texttt{maxPeak}, that is the highest smoothed
probe level in the enriched region. 
...
Figure \ref{Ringo-plotPeak} displays an identified enriched region, 
which is located upstream of the \texttt{Nudt2} gene. 
This peak was obvious in plots of the 
...
Special thanks to Jenny J. Fischer and Silke Sperling
