...
which files belong to which sample\footnote{You will probaby have to
manually construct such a text file for your own data. 
The \texttt{scripts} directory of the package contains a script
...
The user can alternatively supply raw two-color microarray readouts
from other platforms in \Rclass{RGList} format,
and the other functions of \Rpackage{Ringo} can work equally well on
that data.
...
file of all transcripts on human chromose 9 annotated in the 
